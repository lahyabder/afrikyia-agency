...
export default function AdminAboutPage() {
    ...
    // Data State
    const [content, setContent] = useState<AboutTranslations>({
        ar: arMessages.about,
        fr: frMessages.about,
        en: enMessages.about
    });
        ...
        <div className="max-w-4xl mx-auto space-y-8 animate-fade-in-up pb-20">
            ...
            <div className={`flex flex-col md:flex-row justify-between items-center bg-black/40 border border-white/5 p-6 rounded-2xl gap-6 ${isRTL ? 'flex-row' : 'flex-row'}`}>
                <div className="flex items-center gap-4">
                    <div>
                        <h1 className="text-xl font-bold">{t.admin.menu.about}</h1>
                        <p className="text-xs text-white/70">تعديل نصوص قسم من نحن</p>
                    </div>
                </div>
            </div>
