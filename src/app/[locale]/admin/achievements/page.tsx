...
export default function AdminPage() {
    ...
    useEffect(() => {
        ...
        const loadAchievements = async () => {
            try {
                // Clear old legacy local storage caches just to be safe
                localStorage.removeItem('afrikyia-achievements');
                localStorage.removeItem('afrikyia-achievements-modified');
    ...
    const handleDelete = async (itemToDelete: AchievementItem) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                showNotification('error', 'تعذر الحذف في السيرفر.');
            }
        } catch (err) {
                ...
                <div className={`flex flex-col md:flex-row justify-between items-center bg-black/40 border border-white/5 p-6 rounded-2xl gap-6 ${isRTL ? 'flex-row' : 'flex-row'}`}>
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-white/70">{t.admin.achievements.subtitle}</p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex items-center gap-4 hover:border-white/20 transition-all">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-white/70 font-medium">{t.admin.achievements.totalAchievements}</div>
                        </div>
                    </div>
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex items-center gap-4 hover:border-white/20 transition-all">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-white/70 font-medium">{t.admin.achievements.websites}</div>
                        </div>
                    </div>
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex items-center gap-4 hover:border-white/20 transition-all">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-white/70 font-medium">{t.admin.achievements.activities}</div>
                        </div>
                    </div>
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex items-center gap-4 hover:border-white/20 transition-all">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-white/70 font-medium">{t.admin.achievements.works}</div>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="relative w-full max-w-2xl bg-[#0f0f0f] border border-white/10 rounded-2xl overflow-hidden shadow-2xl animate-scale-up flex flex-col max-h-[90vh]">
                            ...
                            <div className="p-6 border-b border-white/10 flex justify-between items-center bg-black/50">
                                <div>
                                    ...
                                    <p className="text-xs text-white/60">يرجى إدخال معلومات العمل بثلاث لغات لضمان الترجمة</p>
                                </div>
                                <button 
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-4 items-center">
                                        ...
                                        <div className="w-full sm:w-36 aspect-video bg-black/40 border border-white/10 rounded-xl flex items-center justify-center overflow-hidden relative">
                                            {image ? (
                                                ...
                                                <span className="text-[10px] text-white/70 text-center px-2">لا توجد صورة</span>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex-1 w-full text-right sm:text-right">
                                            ...
                                            <p className="text-[10px] text-white/60 mt-2">يرجى رفع صورة مناسبة للمشروع وبحجم لا يتعدى 2MB.</p>
                                        </div>
                                    </div>
                                </div>
